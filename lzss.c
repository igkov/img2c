...
unsigned char
		text_buf[N + F - 1];	/* ring buffer of size N, with extra F-1 bytes to facilitate string comparison */
...
void _putc(int data, STREAM_S *stream) {
...
void lzss_pack(unsigned char *input, int sizein, unsigned char *output, int *sizeout) {
	...
	soutfile.data = output;
	soutfile.offset = 0;
	soutfile.size = *sizeout;
